...
def time_check_error(connect, current_account):
    ...
    run_at = nowtime + timedelta(seconds=20)
    if now > run_at:
        command = update(history_account).values({"success": "Ошибка. Время на выаполнение закончилось", "date_expiration": run_at}).where(history_account.c.id == current_account[0])
